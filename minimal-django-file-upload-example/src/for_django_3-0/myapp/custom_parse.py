...
def parse(filename):
    ...
    def write_result_to_excel(final_dict, body_colors, rn_dict):
        ...
        wbname = protocol + "_" + \
            rn_dict["Name"].replace(" ", "-") + "_" + rn_dict["Date"] + ".xlsx"
        folder = os.path.join(proot, "media/results")
        nm = folder + "/" + wbname
        forzip["res"].append(nm)
        print("File name:", wbname)
        wb = xw.Workbook(folder + "/" + wbname)
        ws = wb.add_worksheet()
        ...
        ws.write(5, 3, "Score : " +
                 str(results["score"]) + " / " + results["score_txt"])
        ...
        for i in range(len(results["training"]["Coordination & Proprioception"])):
            height.append(i)
            ws.write(9+i, 2, results["training"]
                     ["Coordination & Proprioception"][i])
    ...
    def write_results_to_pdf(final_dict, body_colors, rn_dict):

        # Should have final dict, body colors, and rn dict
    ...
    def generate_single_result(row):
        ...
        if protocol == "UEFA CORE":
            r_dict = dict.fromkeys(
                list(static.uefa_core.keys()) + ["Additional Information"], 0)
        elif protocol == "UEFA20":
            r_dict = dict.fromkeys(list(static.uefa_core.keys(
            )) + list(static.uefa_20.keys()) + ["Additional Information"], 0)
        elif protocol == "CC25":
            r_dict = dict.fromkeys(list(static.uefa_core.keys(
            )) + list(static.uefa_20.keys()) + list(static.cc25.keys()) + ["Additional Information"], 0)
        ...
        for key in rn_dict:
            ...
            if static.types[key] == "1":
                ...
                t_score = rn_dict[key]["Score"]["L"] + \
                    rn_dict[key]["Score"]["R"]
                body_part = static.body_parts[key]
                if body_part in body_colors:
                    # print(body_colors[body_part])
                    # print(rn_dict[key])
                    body_colors[body_part]["L"].append(
                        rn_dict[key]["Score"]["L"])
                    body_colors[body_part]["R"].append(
                        rn_dict[key]["Score"]["R"])
                else:
                    body_colors[body_part] = {
                        "C": [],
    ...
    def write_result(res):
        ...
        folder = os.path.join(proot, "media/results")
        wb = xw.Workbook(folder + "/" + wbname + ".xlsx")
        nam = folder + "/" + wbname
        forzip["name"] = nam
        nm = folder + "/" + wbname + ".xlsx"
        forzip["res"].append(nm)
        ws = wb.add_worksheet()
        # Header of the file
        ws.write(0, 0, protocol)
